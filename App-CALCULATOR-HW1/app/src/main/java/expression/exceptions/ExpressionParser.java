...
public class ExpressionParser<N> implements CharSource, ParserForGeneric<N> {
    ...
    private void parseConst(StringBuilder sb) throws OverflowException {
        while (hasNext() && (Character.isDigit(now()) || now() == '.' ||
                (now() == 'E' && (
                        (checkNext(1) == '-' && Character.isDigit(checkNext(2))) ||
                                Character.isDigit(checkNext(1))))
        )
        ) {
            if (now() == 'E' && checkNext(1) == '-') sb.append(next());
            sb.append(next());
        }
        try {
...
//                prevExpEn = nowExpEn;
    ...
    private char checkNext(int plus) {
        if ((position + plus) >= line.length()) return 0;
        return line.charAt(position + plus);
    }
